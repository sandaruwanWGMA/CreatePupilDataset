...
def main():
    ...
    # Custom I/O folders for simpler batch use
    parser.add_argument('--input_dir', type=str, default=None, help='Folder with images to process (overrides folder_gt/folder_lq based on task)')
    parser.add_argument('--output_dir', type=str, default=None, help='Folder to save processed images (overrides default results path)')
    ...
    # Support nested folders (e.g., left/right). Gather all files recursively.
    image_paths = [p for p in glob.glob(os.path.join(folder, '**', '*'), recursive=True) if os.path.isfile(p)]
    ...
    for idx, path in enumerate(sorted(image_paths)):
        ...
        # Preserve relative subfolder structure (e.g., left/right)
        rel_subdir = os.path.relpath(os.path.dirname(path), folder)
        out_dir = os.path.join(save_dir, rel_subdir)
        os.makedirs(out_dir, exist_ok=True)
        cv2.imwrite(os.path.join(out_dir, os.path.basename(path)), output)
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        save_dir = args.output_dir or f'results/swinir_{args.task}_x{args.scale}'
        folder = args.input_dir or args.folder_gt
        border = args.scale
        window_size = 8
    ...
    # 003 real-world image sr
        ...
        if args.large_model:
            save_dir += '_large'
        folder = args.input_dir or args.folder_lq
        border = 0
        window_size = 8

    # 004 grayscale image denoising/ 005 color image denoising
    elif args.task in ['gray_dn', 'color_dn']:
        save_dir = args.output_dir or f'results/swinir_{args.task}_noise{args.noise}'
        folder = args.input_dir or args.folder_gt
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = args.output_dir or f'results/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.input_dir or args.folder_gt
        border = 0
        window_size = 7
...
def get_image_pair(args, path):
    ...
    # 004 grayscale image denoising (load gt image and generate lq image on-the-fly)
        ...
        if args.folder_gt is None:
            # Real noisy image provided, no GT available
            img_gt = None
            img_lq = cv2.imread(path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
            img_lq = np.expand_dims(img_lq, axis=2)
        else:
            # Synthetic noise generation using GT
